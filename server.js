...
const ALLOWED_ORIGINS = [
  'https://priceaction.it.com',
  'https://www.priceaction.it.com',
  'http://localhost:3000',
  'http://127.0.0.1:3000'
];
...
function validateSymbol(sym) {
  ...
  return /^[A-Z0-9!\-_]{1,20}$/i.test(sym.trim());
}

function validateTimeframe(tf) {
  const valid = ['1m','5m','15m','30m','1H','2H','4H','6H','8H','12H','1D','3D','1W','1M'];
...
function validateTradeMode(mode) {
  return ['scalp','dayTrade','swing'].includes(mode);
}
...
app.post('/api/analyze', authMiddleware, requirePlan, async (req, res) => {
  ...
  // Validate inputs
  if (symbol && !validateSymbol(symbol)) return res.status(400).json({ error: 'Invalid symbol' });
  if (timeframe && !validateTimeframe(timeframe)) return res.status(400).json({ error: 'Invalid timeframe' });
  if (tradeMode && !validateTradeMode(tradeMode)) return res.status(400).json({ error: 'Invalid tradeMode' });
  const key = process.env.ANTHROPIC_API_KEY;
...
app.post('/api/analyze-live', authMiddleware, requirePlan, async (req, res) => {
  ...
  if (!symbol || !validateSymbol(symbol)) return res.status(400).json({ error: 'Valid symbol required (alphanumeric, 1-20 chars)' });
